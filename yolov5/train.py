...
from utils.attribute_loss import ComputeAttributeLoss
from models.attribute_head import AttributeHead
...
def train(hyp, opt, device, callbacks):
    ...
    train_loader, dataset = create_dataloader(
        ...
        rect=opt.rect,
        rank=LOCAL_RANK,
        attributes_path=opt.attributes,
        workers=workers,
    ...
    # Initialize attribute head if attributes are provided
    attr_head = None
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                # Attribute loss (if enabled)
                attr_loss = torch.tensor(0.0, device=device)
                if attr_head is not None and opt.attributes:
                    # Get attribute targets from dataset
...
def parse_opt(known=False):
    ...
    parser.add_argument("--attributes", type=str, default=None, help="path to attributes mapping json")
