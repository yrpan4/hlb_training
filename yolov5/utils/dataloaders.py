...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        if cache_images:
            ...
            # Load attribute mapping (optional) and build per-image attribute vectors
            # attributes_mapping: mapping from S name (or id) -> {"id": int, "symmetry": str, "vein_color": str}
            self.attributes_by_id = {}
            self.image_attrs = [None] * len(self.im_files)  # per-image attribute vectors (sym_mask, sym_label, vein_mask, vein_label)
            if attributes_path:
                try:
                    with open(attributes_path, 'r', encoding='utf-8') as f:
                        attrs = json.load(f)
    ...
    def get_attrs(self, paths):
        """Return per-image attribute vectors for a batch of image paths.

        Args:
            paths (List[str]): list of image file paths in the batch (in same order as returned by dataloader)

        Returns:
            torch.Tensor: tensor of shape (B, 4) with columns [sym_mask, sym_label, vein_mask, vein_label]
        """
        out = []
        # map path -> index
        path_to_index = {p: i for i, p in enumerate(self.im_files)}
        for p in paths:
            idx = path_to_index.get(p, None)
            if idx is None:
                # try normalized paths
                try:
                    idx = path_to_index.get(str(Path(p).resolve()), None)
                except Exception:
                    idx = None
            if idx is None:
                out.append(np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32))
            else:
                vec = self.image_attrs[idx]
                if vec is None:
                    out.append(np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32))
                else:
                    out.append(vec)
        return torch.from_numpy(np.stack(out, 0))
